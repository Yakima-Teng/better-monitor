...
(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.BetterMonitor = factory());
    ...
    const updateStore = (config) => Object.assign(store, config);
    ...
    // 前端域名
    const FRONTEND_DOMAIN = 'https://www.verybugs.com';
    // export const FRONTEND_DOMAIN = 'http://localhost:8989'
    // 后端域名
    const BACKEND_DOMAIN = FRONTEND_DOMAIN;

    // 校验请求参数是否在黑名单中，如果返回false表示在黑名单中，不继续后续上报操作
    const validateBugRequestData = (requestData) => {
        ...
        const selfBlackList = [FRONTEND_DOMAIN];
    ...
    const getUserId = () => {
        ...
        if (typeof store.getUserId === 'function') {
            const gettingUserId = store.getUserId();
    ...
    const addAction = (params) => {
        ...
        const selfBlackList = [BACKEND_DOMAIN];
    ...
    const initLogPlugin = () => {
        ...
        XMLHttpRequest.prototype.send = function (body) {
            ...
            xhrInstance.addEventListener('loadend', (e) => {
    ...
    const handlerFuncForJsUnhandledRejection = (e) => {
        // eslint-disable-next-line no-console
        console.error(e);
        try {
            e.preventDefault();
    ...
    const tryInitSettingAutomatically = () => {
        const elemScripts = document.querySelectorAll('script');
        let targetElem = null;
        for (let i = 0, len = elemScripts.length; i < len; i++) {
            const elem = elemScripts[i];
