...
import {
  BACKEND_DOMAIN, buildDate, buildVersion
} from '@/constant'
import isString from '@/common/isString'

// 校验请求参数是否在黑名单中，如果返回false表示在黑名单中，不继续后续上报操作
export const validateBugRequestData = (requestData: BetterMonitor.IParamsAddBug): boolean => {
  const { blackList } = getStore()

  const matchKeyword = (keyword: string | RegExp): boolean => {
    if (isString(keyword)) {
      return requestData.message.includes(keyword) || requestData.message.includes(keyword) || requestData.stack.includes(keyword)
    }
  ...
  if (!requestData.pageUrl.includes('verybugs.com') && selfBlackList.some(matchKeyword)) {
    return false
  }
  // 没有具体错误信息，上报也没有意义
